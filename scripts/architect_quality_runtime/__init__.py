...
def _render_diagnostics(values: Iterable[Any]) -> list[str]:
    return [item.render() for item in values]
...
def _blocking_diagnostics(result: dict[str, Any]) -> tuple[Any, ...]:
    diagnostics: list[Any] = []
    for item in result.get("blocking_issues", []):
        if not isinstance(item, dict):
            continue
        diagnostics.append(
            _core.RuntimeDiagnostic(
                str(item.get("issue_id") or "RUNTIME_STAGE_BLOCKED"),
                str(item.get("reason") or "Stage evaluation was blocked"),
...
def _incomplete_diagnostic() -> Any:
    return _core.RuntimeDiagnostic(
        "RUNTIME_RUN_INCOMPLETE",
        "complete replay must pass every mandatory Stage",
        path="run",
    )
...
class ReplayOutcome:
    ...
    def to_public(self) -> dict[str, Any]:
        results = [_copy(item) for item in self.results]
        return {
            "status": self.status,
            "errors": sorted(set(_render_diagnostics(self.diagnostics))),
            "diagnostics": [_diagnostic_dict(item) for item in self.diagnostics],
            "results": results,
            "stages_visited": [item["stage_id"] for item in results],
            "all_required_stages_visited": [item["stage_id"] for item in results]
            == list(self.stage_order),
            "terminal_stage": self.stage_order[-1] if self.stage_order else None,
            "run_state": _copy(self.run_state),
        }
...
def _replay_outcome(
    ...
    *,
    ...
    try:
        ...
        return ReplayOutcome("invalid", tuple(exc.diagnostics), (), None, stage_order)
    ...
    if not outputs:
        ...
        return ReplayOutcome("valid", (), (), None, stage_order)

    sequence = _sequence_diagnostics(outputs, order, require_terminal=require_terminal)
    if sequence:
        return ReplayOutcome("invalid", tuple(sequence), (), None, stage_order)
    ...
    for output in outputs:
        try:
            ...
            if require_terminal and len(results) != len(order):
                diagnostics.append(_incomplete_diagnostic())
            return ReplayOutcome(
                "invalid", tuple(diagnostics), tuple(results), state, stage_order
            )
        results.append(result)
        if result["stage_status"] != "pass":
            diagnostics = list(_blocking_diagnostics(result))
            if require_terminal and len(results) != len(order):
                diagnostics.append(_incomplete_diagnostic())
            return ReplayOutcome(
                "invalid", tuple(diagnostics), tuple(results), state, stage_order
    ...
    if require_terminal and len(results) != len(order):
        return ReplayOutcome(
            "invalid", (_incomplete_diagnostic(),), tuple(results), state, stage_order
        )
    return ReplayOutcome("valid", (), tuple(results), state, stage_order)
...
class RuntimeSession:
    ...
    def _rejected(
        ...
        *,
        ...
        return {
            "accepted": False,
            "stage_result": _copy(stage_result),
            "diagnostics": _render_diagnostics(values),
            "diagnostic_details": [_diagnostic_dict(item) for item in values],
    ...
    def _advance_strict(self, stage_output: dict[str, Any]) -> dict[str, Any]:
        ...
        if observed == self._order[-1]:
            ...
            self._state = _copy(replay.run_state)
            return {
                "accepted": True,
        ...
        if result["stage_status"] != "pass":
            return self._rejected(
                _blocking_diagnostics(result), stage_result=result, state=self._state
            )
        self._outputs.append(candidate)
    ...
    def advance(self, stage_output: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            return self._rejected(
                exc.diagnostics, stage_result=None, state=self._state
            )
...
def resume_run(
    ...
    *,
    ...
    if replay.status != "valid":
        raise _core.RunSequenceValidationError(replay.diagnostics)
    return RuntimeSession(
        root=Path(repository_root),
        run_context=run_context,
        git_provider=_provider_for_context(run_context, git_provider),
        outputs=outputs,
        results=[_copy(item) for item in replay.results],
        state=_copy(replay.run_state),
    )
    ...
    if not isinstance(result, dict) or result.get("stage_id") != stage_id:
        raise _core.StageOutputValidationError(
            _core.RuntimeDiagnostic(
                ...
                "Stage evaluation failed without a Stage Result",
                stage_id=stage_id,
            )
        )
