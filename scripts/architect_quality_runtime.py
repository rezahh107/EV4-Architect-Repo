...
class RunContext:
    ...
    def __post_init__(self) -> None:
        if self.source_kind not in SOURCE_KINDS:
            raise ValueError(f"Unsupported RunContext.source_kind: {self.source_kind!r}")
        if any(not isinstance(item, str) or not item for item in self.verified_evidence_refs):
            raise ValueError("RunContext.verified_evidence_refs must contain non-empty strings")
...
class SubprocessGitProvider:
    ...
    def provenance(self, root: Path) -> Any:
        ...
        if top != root.resolve():
            raise ValueError("Runtime Git provider requires the repository root")
        ...
        if dirty:
            raise ValueError("Runtime Git provider requires a clean checkout")
...
def derive_stage_result_authority(schema: dict[str, Any]) -> StageResultAuthorityClassification:
    ...
    forbidden = (
        evaluator_top_level | frozenset(nested_fields) | EXPLICIT_LEGACY_AUTHORITY_ALIASES
...
def load_stage_output_schema(root: Path, result_schema: dict[str, Any]) -> dict[str, Any]:
    ...
    if derived != mirrored:
        raise ValueError(
            "Runtime and Base Schema forbidden sets differ; classification mirror update required: "
...
def _schema_issues(stage_output: dict[str, Any], schema: dict[str, Any], stage_id: str) -> list[dict[str, Any]]:
    found = sorted(
...
def _apply_unknowns(
    output: dict[str, Any], state: dict[str, Any], stage: dict[str, Any],
    run_context: RunContext, issues: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    ...
    for item in output.get("unknown_introductions", []):
        ...
        if not uid or not str(statement or "").strip() or uid in by_id:
            issues.append(_issue(
                ...
                "Unknown identity must be new and have a statement", stage["stage_id"],
            ))
            continue
        record = {
            "unknown_id": uid, "statement": statement, "status": "active",
    ...
    for change in output.get("unknown_resolutions", []):
        ...
        if (
            not record or record["status"] != "active" or kind not in RESOLUTION_TYPES
        ...
        if record.get("downstream_critical") and (
            not isinstance(ref, str) or ref not in run_context.verified_evidence_refs
        ):
        ...
        record["resolution"] = {
            "resolution_type": kind, "note": note,
            "resolved_at_stage": stage["stage_id"], "evidence_ref": ref,
        }
    return ledger
...
def _evaluate_project_gate(
    output: dict[str, Any], state: dict[str, Any], root: Path,
    run_context: RunContext, git_provider: GitProvider | None,
    issues: list[dict[str, Any]],
) -> dict[str, Any] | None:
    if "project_gate_payload" in output:
        issues.append(_issue(
            "RUNTIME_CALLER_PROJECT_GATE_PAYLOAD_FORBIDDEN",
            "The official terminal Payload is assembled by Runtime from the evaluated Run",
            "/project-gate-export",
        ))
        return None
    scripts = root / "scripts"
    ...
    try:
        ...
        export, hashes = contracts.build_export(
            payload, _producer_provenance(root, git_provider), state["run_id"],
        ...
        if export.get("run_id") != state["run_id"]:
            raise ValueError("Generated export belongs to another Run")
        if run_context.synthetic and allowed:
            raise ValueError("Synthetic Runtime context produced an allowed real handoff")
        if not run_context.synthetic and eligibility["would_allow"] and not allowed:
            raise ValueError("Eligible live Runtime context did not produce an allowed handoff")
    except Exception as exc:
        issues.append(_issue(
            "RUNTIME_PROJECT_GATE_VALIDATION_FAILED",
            f"Canonical Project Gate validation failed: {type(exc).__name__}: {exc}",
            "/project-gate-export",
        ))
        return None
    return {
        ...
        "execution_context": {
            "source_kind": run_context.source_kind, "synthetic": run_context.synthetic,
        },
...
def evaluate_stage(
    stage_id: str, stage_output: dict[str, Any], run_state: dict[str, Any], *,
    root: Path = ROOT, run_context: RunContext | None = None,
    ...
    issues: list[dict[str, Any]] = []
    issues.extend(_schema_issues(stage_output, stage_output_schema, stage_id))
    for field in sorted(authority.forbidden_top_level_stage_output_fields.intersection(stage_output)):
        code = (
            "RUNTIME_CALLER_PROJECT_GATE_PAYLOAD_FORBIDDEN"
            if field == "project_gate_payload" else "RUNTIME_CALLER_AUTHORITY_FIELD_FORBIDDEN"
        )
        issues.append(_issue(code, f"Caller-authored authority field is forbidden: {field}", stage_id))
    if stage_output.get("run_id") != run_state.get("run_id") or stage_output.get("stage_id") != stage_id:
        issues.append(_issue("RUNTIME_STAGE_OUTPUT_IDENTITY_MISMATCH", "Stage Output identity mismatch", stage_id))
    if stage_output.get("stage_version") != stage.get("stage_version"):
        issues.append(_issue("RUNTIME_STAGE_OUTPUT_VERSION_MISMATCH", "Stage Output version mismatch", stage_id))
    for item in stage_output.get("blockers", []):
        if isinstance(item, dict) and item.get("issue_id") not in INFRA_BLOCKERS:
            issues.append(_issue(
                str(item.get("issue_id")), str(item.get("reason")),
                item.get("repair_stage", stage_id), str(item.get("kind", "quality")),
            ))
    ...
    if stage_id in {"/final-audit", "/handoff-export", "/project-gate-export"} and any(
        isinstance(item, dict) and item.get("status") == "active" and item.get("downstream_critical")
    ...
    issues.extend(
        _issue(item["issue_id"], item["reason"], item.get("repair_stage", stage_id))
    ...
    if stage_id == "/project-gate-export" and not issues:
        if run_context is None:
            issues.append(_issue(
                ...
                "The terminal Stage requires an explicit Runtime-owned RunContext", stage_id,
            ))
        else:
            project_gate_export = _evaluate_project_gate(
                stage_output, next_state, root, context, git_provider, issues,
            )
    ...
    status = (
        "pass" if not public_issues else
        "needs_input" if issues and all(item.get("_kind") == "user_input" for item in issues)
        else "blocked"
    )
    ...
    for item in next_state.get("unknown_ledger", []):
        value = {
            "unknown_id": item["unknown_id"], "statement": item["statement"],
            "status": item["status"], "evidence_refs": item.get("evidence_refs", []),
            "downstream_critical": bool(item.get("downstream_critical")),
        }
    ...
    result = {
        ...
        "stage_status": status,
        "completion_class": guard.completion_class,
        ...
        "runtime_context": {
            ...
            "legacy_check_results_ignored": list(guard.legacy_result_fields_ignored),
        },
    ...
    schema_errors = sorted(
        Draft202012Validator(result_schema).iter_errors(result), key=lambda error: list(error.absolute_path)
    )
    if schema_errors:
    ...
    if status == "pass":
        ...
        next_state["evaluated_stage_outputs"] = [
            *run_state.get("evaluated_stage_outputs", []), copy.deepcopy(stage_output)
        ]
        next_state["derived_stage_results"] = [
            *run_state.get("derived_stage_results", []), copy.deepcopy(result)
        ]
    else:
        next_state = copy.deepcopy(run_state)
...
def evaluate_run(
    ...
    root: Path = ROOT, require_terminal: bool = True,
    git_provider: GitProvider | None = None,
) -> dict[str, Any]:
    ...
    for output in outputs:
        try:
            result, state = evaluate_stage(
                output["stage_id"], output, state, root=root,
        ...
        if result["stage_status"] != "pass":
            errors.extend(
                f"{result['stage_id']}: {item['issue_id']}" for item in result["blocking_issues"]
            )
            break
...
def apply_partial_rerun(run_state: dict[str, Any], earliest_stage: str, *, root: Path = ROOT) -> dict[str, Any]:
    ...
    invalidated = [
        stage for stage in state.get("completed_stages", []) if order.index(stage) >= index
    ]
    ...
    state["evaluated_stage_outputs"] = [
        ...
        if item.get("stage_id") in order and order.index(item["stage_id"]) < index
    ]
    state["derived_stage_results"] = [
        item for item in state.get("derived_stage_results", [])
        if item.get("stage_id") in order and order.index(item["stage_id"]) < index
    ]
