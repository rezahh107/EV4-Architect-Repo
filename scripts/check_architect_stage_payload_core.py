...
class ArchitectPayloadValidator:
    ...
    def _semantic(self,v:dict):
        ...
        if v.get("payload_status")=="complete" and classification.transition_blockers:
            out.append(D("A_R05_TRANSITION_BLOCKER_STATUS_MISMATCH","insufficient_evidence","Complete payloads must not carry unresolved evidence that blocks Architect Payload acceptance or CE transition.","$.unresolved_evidence","A-R05",transition_blocker_count=len(classification.transition_blockers)))
    ...
    def _result(self,value,diags):
        ...
        status="invalid" if any(d.severity=="error" for d in ordered) else "insufficient_evidence" if any(d.severity=="insufficient_evidence" for d in ordered) or isinstance(value,dict) and value.get("payload_status")=="insufficient_evidence" else "valid"
