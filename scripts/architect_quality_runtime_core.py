...
BASE_STAGE_OUTPUT_SCHEMA_PATH = (
    ROOT / "schemas/architect-conversational-stage-output-base.v1.schema.json"
)
...
class RunContext:
    source_kind: Literal[
        "live_conversation", "fixture", "example", "test_vector"
    ]
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            ...
            for item in self.verified_evidence_refs
        ):
            diagnostics.append(
                RuntimeDiagnostic(
...
class SubprocessGitProvider:
    ...
    def provenance(self, root: Path) -> Any:
        ...
        if top != root.resolve():
            raise RuntimeEnvironmentError(
                RuntimeDiagnostic(
                    "RUNTIME_GIT_ROOT_MISMATCH",
                    "Runtime Git provider requires the repository root",
                )
            )
        ...
        ref = (
            ...
            else "detached-exact-head"
        )
        sha = self._git(root, "rev-parse", "HEAD")
        if not base.SHA40.fullmatch(sha):
            raise RuntimeEnvironmentError(
                RuntimeDiagnostic(
                    "RUNTIME_GIT_HEAD_INVALID",
                    "Runtime Git provider requires a full HEAD SHA",
                )
            )
        dirty = self._git(
            root,
        ...
        if dirty:
            raise RuntimeEnvironmentError(
                RuntimeDiagnostic(
                    "RUNTIME_GIT_CHECKOUT_DIRTY",
                    "Runtime Git provider requires a clean checkout",
                )
            )
...
def derive_stage_result_authority(
    schema: dict[str, Any],
    ...
    for name in EVALUATOR_DERIVED_NESTED_DEFS:
        if name not in defs:
            raise ValueError(
                f"Stage Result Schema.$defs.{name} is required for authority classification"
            )
        nested_fields.update(
            _schema_properties(
                defs[name], location=f"Stage Result Schema.$defs.{name}"
            )
        )
...
def _validate_caller_authority_classification(
    schema: dict[str, Any],
) -> None:
...
def _base_schema_authority_fields(
    schema: dict[str, Any],
) -> frozenset[str]:
    values = (
        schema.get("$defs", {})
    ...
    if (
        ...
        raise ValueError(
            "Conversational Base Schema caller authority mirror is invalid"
        )
...
def validate_authority_documents(
    ...
    if not isinstance(model, dict):
        raise ValueError(
            "normal_run_continuation must be a JSON object"
        )
    ...
    if model.get("serialized_stage_result_authorizes") is not False:
        raise ValueError(
            "serialized Stage Results must not authorize continuation"
        )
    for key in (
...
def load_authority(
    ...
    assembler = importlib.import_module(
        "architect_runtime_payload_assembler"
    )
    assembler.validate_derivation_authority(root)
    return manifest, schema


def load_stage_output_schema(
    root: Path, result_schema: dict[str, Any]
) -> dict[str, Any]:
    schema = _load(root / BASE_STAGE_OUTPUT_SCHEMA_PATH.relative_to(ROOT))
    Draft202012Validator.check_schema(schema)
    derived = derive_stage_result_authority(
        result_schema
    ).forbidden_top_level_stage_output_fields
    mirrored = _base_schema_authority_fields(schema)
    if derived != mirrored:
        raise ValueError(
            "Runtime and Base Schema forbidden sets differ; classification mirror "
            f"update required: missing={sorted(derived - mirrored)}, "
            f"extra={sorted(mirrored - derived)}"
        )
    return schema
...
def _schema_diagnostics(
    stage_output: Any, schema: dict[str, Any], stage_id: str
) -> list[RuntimeDiagnostic]:
    found = sorted(
        Draft202012Validator(schema).iter_errors(stage_output),
        key=lambda error: (
    ...
    return [
        RuntimeDiagnostic(
            "RUNTIME_STAGE_OUTPUT_SCHEMA_INVALID",
            error.message,
...
def _apply_unknowns(
    ...
    for item in output.get("unknown_introductions", []):
        ...
        statement = (
            item.get("statement") if isinstance(item, dict) else None
        )
        if (
            not uid
            ...
            issues.append(
                _issue(
                    "RUNTIME_UNKNOWN_INTRODUCTION_INVALID",
                    "Unknown identity must be new and have a statement",
                    stage["stage_id"],
                )
            )
            continue
    ...
    for change in output.get("unknown_resolutions", []):
        if not isinstance(change, dict):
            issues.append(
                _issue(
                    "RUNTIME_UNKNOWN_RESOLUTION_INVALID",
                    "Unknown resolution must be an object",
                    stage["stage_id"],
                )
            )
            continue
        uid = change.get("unknown_id")
        kind = change.get("resolution_type")
        note = change.get("note")
        record = by_id.get(uid)
        if (
            not record
            or record["status"] != "active"
            or kind not in RESOLUTION_TYPES
            or not str(note or "").strip()
        ):
            issues.append(
                _issue(
                    "RUNTIME_UNKNOWN_RESOLUTION_INVALID",
                    f"Invalid explicit resolution for {uid}",
                    stage["stage_id"],
                )
            )
            continue
        if (
            kind == "not_applicable"
            and not stage.get(
                "allow_not_applicable_unknown_resolution"
            )
        ):
            issues.append(
                _issue(
                    "RUNTIME_UNKNOWN_NOT_APPLICABLE_FORBIDDEN",
                    f"not_applicable is forbidden at {stage['stage_id']}",
                    stage["stage_id"],
                )
            )
            continue
        ref = change.get("evidence_ref")
        if record.get("downstream_critical") and (
            not isinstance(ref, str)
            or ref not in run_context.verified_evidence_refs
        ):
            issues.append(
                _issue(
                    "RUNTIME_CRITICAL_UNKNOWN_EVIDENCE_REQUIRED",
                    f"Resolvable evidence is required for {uid}",
                    stage["stage_id"],
                )
            )
            continue
...
def _producer_provenance(
    root: Path, git_provider: GitProvider | None
) -> Any:
    return (git_provider or SubprocessGitProvider()).provenance(root)


def _expected_issues(
    exc: ArchitectRuntimeExpectedError, stage_id: str
) -> list[dict[str, Any]]:
    return [
        _issue(item.code, item.message, item.stage_id or stage_id)
...
def _evaluate_project_gate(
    ...
    contracts = importlib.import_module(
        "architect_project_gate_exporter.contracts"
    )
    runtime_gate = importlib.import_module(
        "architect_runtime_project_gate"
    )
    ...
    try:
        payload = assembler.assemble_architect_stage_payload(
            run_state=state, source_kind=run_context.source_kind
        )
        validation = runtime_gate.validate_payload(root, payload)
        eligibility = eligibility_module.derive_handoff_eligibility(
            payload
        )
        export, hashes = contracts.build_export(
            payload,
            _producer_provenance(root, git_provider),
            state["run_id"],
        ...
        issues.extend(
            _expected_issues(exc, "/project-gate-export")
        )
        return None
    except contracts.ExportError as exc:
        issues.append(
            _issue(exc.code, exc.reason, "/project-gate-export")
        )
        return None
    return {
...
def evaluate_stage(
    ...
    *,
    ...
    stage_output_schema = load_stage_output_schema(
        root, result_schema
    )
    ...
    if not stage or run_state.get("current_stage") != stage_id:
        raise RunSequenceValidationError(
            RuntimeDiagnostic(
                "RUNTIME_STAGE_SEQUENCE_MISMATCH",
                "Run State and Stage do not match",
                stage_id=stage_id,
            )
        )
    diagnostics = _schema_diagnostics(
        stage_output, stage_output_schema, stage_id
    )
    if isinstance(stage_output, dict):
        for field in sorted(
            authority.forbidden_top_level_stage_output_fields.intersection(
                stage_output
            )
        ):
            code = (
            ...
            diagnostics.append(
                RuntimeDiagnostic(
                    code,
                    f"Caller-authored authority field is forbidden: {field}",
                    path=field,
                    stage_id=stage_id,
                )
            )
        if (
            stage_output.get("run_id") != run_state.get("run_id")
            or stage_output.get("stage_id") != stage_id
        ):
            diagnostics.append(
                RuntimeDiagnostic(
                    "RUNTIME_STAGE_OUTPUT_IDENTITY_MISMATCH",
                    "Stage Output identity mismatch",
                    stage_id=stage_id,
                )
            )
        if stage_output.get("stage_version") != stage.get(
            "stage_version"
        ):
            diagnostics.append(
                RuntimeDiagnostic(
                    "RUNTIME_STAGE_OUTPUT_VERSION_MISMATCH",
                    "Stage Output version mismatch",
                    stage_id=stage_id,
                )
            )
    if diagnostics:
    ...
    for item in stage_output.get("blockers", []):
        if (
            ...
            and item.get("issue_id") not in INFRA_BLOCKERS
        ):
            issues.append(
                _issue(
                    str(item.get("issue_id")),
                    str(item.get("reason")),
                    item.get("repair_stage", stage_id),
                    str(item.get("kind", "quality")),
                )
            )

    next_state = copy.deepcopy(run_state)
    next_state["unknown_ledger"] = _apply_unknowns(
        stage_output, next_state, stage, context, issues
    )
    selected = next_state.get("selected_candidate_id")
    locked = bool(next_state.get("selected_candidate_locked"))
    decision = (
        stage_output.get("decision_input")
        if isinstance(stage_output.get("decision_input"), dict)
        else {}
    )
    if (
        stage_id != "/recommend"
        and locked
        and decision.get("selected_candidate_id", selected)
        != selected
    ):
        issues.append(
            _issue(
                "RUNTIME_CANDIDATE_DRIFT",
                "selected_candidate_id changed after lock",
                stage_id,
            )
        )
    if decision.get("architecture_drift") is True:
        issues.append(
            _issue(
                "RUNTIME_ARCHITECTURE_DRIFT",
                "Architecture drift is forbidden",
                stage_id,
            )
        )
    if decision.get("hidden_recommendation") is True:
        issues.append(
            _issue(
                "RUNTIME_HIDDEN_RECOMMENDATION",
                "Hidden recommendation is forbidden",
                stage_id,
            )
        )
    if decision.get("unknown_converted_to_exact") is True:
        issues.append(
            _issue(
                "RUNTIME_UNKNOWN_TO_EXACT",
                "Unknown evidence cannot become an exact value",
                stage_id,
            )
        )

    build_digest = next_state.get("build_tree_digest")
    implementation_digest = next_state.get(
        "implementation_digest"
    )
    ...
    if stage_id != "/final-audit" and findings:
        issues.append(
            _issue(
                "RUNTIME_FINAL_AUDIT_WRONG_STAGE",
                "Final Audit findings belong only to /final-audit",
                stage_id,
            )
        )
    if (
        stage_id
        in {"/final-audit", "/handoff-export", "/project-gate-export"}
        and any(
            isinstance(item, dict)
            and item.get("status") == "active"
            and item.get("downstream_critical")
            for item in next_state.get("unknown_ledger", [])
        )
    ):
        issues.append(
            _issue(
                "RUNTIME_CRITICAL_UNKNOWN_ACTIVE",
                "A downstream-critical unknown remains active",
                stage_id,
            )
        )
    ...
    if not issues and stage_id == "/build-tree":
        build_digest = _digest(content)
        next_state["build_tree_digest"] = build_digest
        approved_digest = build_digest
    if not issues and stage_id == "/implementation" and isinstance(
        content, dict
    ):
    ...
    if stage_id == "/project-gate-export" and not issues:
        if run_context is None:
            issues.append(
                _issue(
                    "RUNTIME_RUN_CONTEXT_REQUIRED",
                    "The terminal Stage requires an explicit Runtime-owned RunContext",
                    stage_id,
                )
            )
        else:
            project_gate_export = _evaluate_project_gate(
                stage_output,
                next_state,
                root,
                context,
            ...
            checks = {
                key: "pass" if project_gate_export else "fail"
                for key in stage["required_quality_checks"]
            }
    ...
    public_issues = [
        ...
        for item in issues
        if item["issue_id"] not in INFRA_BLOCKERS
    ]
    status = (
        "pass"
        if not public_issues
            ...
            if issues
            and all(
                item.get("_kind") == "user_input" for item in issues
            )
            else "blocked"
        )
    )
    ...
    result = {
        ...
        "research_disposition": (
            stage_output.get("research_disposition")
        ...
        "runtime_context": {
            ...
            "legacy_check_results_ignored": list(
                guard.legacy_result_fields_ignored
            ),
        },
        "final_audit_findings": (
            findings if stage_id == "/final-audit" else []
        ),
    ...
    if status == "pass":
        result["completion_class"] = (
            claim_guard.REASONING_COMPLETE
            if stage_id in claim_guard.REASONING_STAGES
            else claim_guard.VALIDATED_PASS
        )
    ...
    if schema_errors:
        raise AssertionError(
            "Derived Stage Result schema failure: "
            + schema_errors[0].message
        )
    if status == "pass":
        next_state["completed_stages"] = [
            *run_state.get("completed_stages", []),
            stage_id,
        ]
        next_state["current_stage"] = next_stage
        next_state["evaluated_stage_outputs"] = [
            *run_state.get("evaluated_stage_outputs", []),
            copy.deepcopy(stage_output),
        ]
        next_state["derived_stage_results"] = [
            *run_state.get("derived_stage_results", []),
...
def _invalid_run(
    ...
    return {
        ...
        "all_required_stages_visited": [
            item["stage_id"] for item in results
        ]
        == order,
        ...
        for output in outputs:
            ...
            if result["stage_status"] != "pass":
                ...
                if require_terminal and len(results) != len(order):
                    errors.append(
                        "run: complete regression must visit every mandatory Stage"
                    )
                return _invalid_run(
                    errors, results, state, order
                )
    except ArchitectRuntimeExpectedError as exc:
...
def validate_stage_result(
    ...
    return [
        *errors,
        "serialized Stage Result is informational only; recompute from Stage Output and Run State",
    ]
...
def apply_partial_rerun(
    ...
    *,
    ...
    state["evaluated_stage_outputs"] = [
        item
        for item in state.get("evaluated_stage_outputs", [])
        if item.get("stage_id") in order
        and order.index(item["stage_id"]) < index
    ]
    state["derived_stage_results"] = [
        item
    ...
    for unknown in state.get("unknown_ledger", []):
        resolved_at = (
            (unknown.get("resolution") or {}).get(
                "resolved_at_stage"
            )
        )
        if resolved_at in order and order.index(resolved_at) >= index:
            unknown.update(
                status="active", resolution=None, evidence_refs=[]
            )
