...
def run_context(source_kind: str = "fixture") -> runtime.RunContext:
...
def test_legacy_not_applicable_result_is_non_authorizing() -> None:
    item = copy.deepcopy(prefinal_outputs()[0])
    ...
    assert "required_input_captured" in result["runtime_context"]["legacy_check_results_ignored"]
...
def test_active_critical_unknown_and_high_final_audit_finding_block_handoff() -> None:
    ...
    critical[0]["unknown_introductions"] = [
...
def test_standalone_validator_rejects_invalid_terminal_structure(tmp_path: Path) -> None:
...
def test_actual_cli_exits_nonzero_for_terminal_runtime_failure(tmp_path: Path) -> None:
    injected = terminal_output()
    injected["project_gate_payload"] = {"fabricated": True}
...
def test_release_upload_set_detects_manifest_check_drift(tmp_path: Path) -> None:
    ...
    assert outcome["status"] == "invalid"
    assert any(
        "Manifest check classification drift" in error
        or "Runtime authority is invalid" in error
        for error in outcome["errors"]
    )
