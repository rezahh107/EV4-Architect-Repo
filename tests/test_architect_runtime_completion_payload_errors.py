...
class FixtureGitProvider:
    def provenance(self, root: Path):
        ...
        return base.GitProvenance("rezahh107/EV4-Architect-Repo", "fixture-exact-head", "7" * 40)
...
def evaluate_prefix(items: list[dict], count: int, *, kind: str = "fixture"):
    ...
    for output in items[:count]:
        result, state = runtime.evaluate_stage(
            ...
            run_context=context(kind), git_provider=FixtureGitProvider(),
        )
...
def evaluate_full(items: list[dict] | None = None, *, kind: str = "fixture"):
    return runtime.evaluate_run(
        items or all_outputs(), root=REPO_ROOT, run_context=context(kind),
...
def test_every_passing_stage_gets_the_truthful_completion_class() -> None:
    ...
    for result in outcome["results"]:
        expected = (
            "reasoning_complete"
            if result["stage_id"] in importlib.import_module("architect_stage_claim_guard").REASONING_STAGES
            else "validated_pass"
        )
...
def test_unsuccessful_stage_never_emits_a_success_completion_class(stage_index: int, status_kind: str) -> None:
    items = all_outputs()
    _, state = evaluate_prefix(items, stage_index)
    candidate = copy.deepcopy(items[stage_index])
    candidate["blockers"] = [
        {
            "issue_id": "TEST_USER_INPUT" if status_kind == "needs_input" else "TEST_BLOCK",
    ...
    result, next_state = runtime.evaluate_stage(
        candidate["stage_id"], candidate, state, root=REPO_ROOT,
        run_context=context(), git_provider=FixtureGitProvider(),
    )
...
def test_stage_result_schema_rejects_success_class_on_blocked_and_needs_input() -> None:
    ...
    schema = json.loads((REPO_ROOT / "schemas/ev4-architect-stage-result.v1.schema.json").read_text(encoding="utf-8"))
...
def test_payload_lineage_rejects_failed_stage_with_success_completion() -> None:
    ...
    with pytest.raises(PayloadDerivationError, match="PAYLOAD_LINEAGE_FALSE_COMPLETION"):
        assembler.assemble_architect_stage_payload(run_state=state, source_kind="fixture")
...
def test_structural_invalidity_is_typed_and_has_no_stage_result() -> None:
    ...
    with pytest.raises(StageOutputValidationError) as caught:
        runtime.evaluate_stage("/intake", item, state, root=REPO_ROOT, run_context=context())
    assert any(item.code == "RUNTIME_STAGE_OUTPUT_SCHEMA_INVALID" for item in caught.value.diagnostics)
    assert state["completed_stages"] == []


def test_evaluate_run_structures_only_expected_domain_failures() -> None:
    outcome = runtime.evaluate_run(
        [], root=REPO_ROOT, run_context=context(), git_provider=FixtureGitProvider()
    )
    assert outcome["status"] == "invalid"
    assert any("RUNTIME_RUN_EMPTY" in error for error in outcome["errors"])


@pytest.mark.parametrize("target", ["stage_predicate", "assembler", "payload_validator", "exporter", "hash_verifier"])
def test_unexpected_programming_defects_propagate(monkeypatch: pytest.MonkeyPatch, target: str) -> None:
    ...
    if target == "stage_predicate":
        ...
        monkeypatch.setattr(guard, "evaluate_claims", lambda **kwargs: (_ for _ in ()).throw(defect))
        state = runtime.initial_run_state(items[0]["run_id"], root=REPO_ROOT)
        with pytest.raises(RuntimeError, match="synthetic programming defect"):
            runtime.evaluate_stage("/intake", items[0], state, root=REPO_ROOT, run_context=context())
        return

    _, state = evaluate_prefix(items, 11)
    if target == "assembler":
        module = importlib.import_module("architect_runtime_payload_assembler")
        monkeypatch.setattr(module, "assemble_architect_stage_payload", lambda **kwargs: (_ for _ in ()).throw(defect))
    elif target == "payload_validator":
        module = importlib.import_module("architect_runtime_project_gate")
        monkeypatch.setattr(module, "validate_payload", lambda *args, **kwargs: (_ for _ in ()).throw(defect))
    elif target == "exporter":
        module = importlib.import_module("architect_project_gate_exporter.contracts")
        monkeypatch.setattr(module, "build_export", lambda *args, **kwargs: (_ for _ in ()).throw(defect))
    else:
        module = importlib.import_module("architect_project_gate_exporter.contracts")
        monkeypatch.setattr(module, "verify_hashes", lambda *args, **kwargs: (_ for _ in ()).throw(defect))
    with pytest.raises(RuntimeError, match="synthetic programming defect"):
        runtime.evaluate_stage(
            "/project-gate-export", items[11], state, root=REPO_ROOT,
            run_context=context(), git_provider=FixtureGitProvider(),
        )
...
def test_required_implementation_intent_blocks_at_owning_stage() -> None:
    ...
    for field in ("class_intent", "class_application_map", "element_mapping"):
        ...
        result, next_state = runtime.evaluate_stage(
            "/implementation", candidate, state, root=REPO_ROOT, run_context=context()
        )
        assert result["stage_status"] == "blocked"
        assert "completion_class" not in result
        assert any(item["issue_id"] == "RUNTIME_STAGE_PREDICATE_FAILED" for item in result["blocking_issues"])
        assert next_state == state
...
def test_missing_conditional_intent_never_creates_a_false_complete_payload(field: str, unresolved_id: str) -> None:
    items = all_outputs()
    ...
    assert any(item["unresolved_id"] == unresolved_id for item in payload["unresolved_evidence"])
...
def test_explicit_empty_decisions_are_distinct_from_omission() -> None:
    ...
    payload = complete["results"][-1]["project_gate_export"]["runtime_issued_payload"]
    ...
    assert payload["architect_intent"]["dynamic_loop_intent"]["status"] == "not_applicable"
    assert not any(item["unresolved_id"].startswith("U-payload-") for item in payload["unresolved_evidence"])
